...
app_to_name_map = {'https://wynk.in/music': "WYNK",
                   ...
                   'https://www.apple.com/': "APPLEMUSIC",
                   'STFD':"SPEEDTEST"}


app_to_file_map = {"https://www.primevideo.com/":"Replay/PRIMEVIDEO",
                   'https://www.netflix.com/': "Replay/NETFLIX",
                   'https://www.youtube.com/': "Replay/YOUTUBE",
                   'https://www.apple.com/': "Replay/APPLEMUSIC",
                   'https://www.spotify.com/': "Replay/SPOTIFY"
}


app_to_clfile_map = {"https://www.primevideo.com/":"Replay/PRIMEVIDEO_CL",
                   'https://www.netflix.com/': "Replay/NETFLIX_CL",
                   'https://www.youtube.com/': "Replay/YOUTUBE_CL",
                   'https://www.apple.com/': "Replay/APPLEMUSIC_CL",
                   'https://www.spotify.com/': "Replay/SPOTIFY_CL"
}
...
def stream_stored_app_data(c, cid, app, tx_window, is_dis_seg, blist):
    import time
    #import gevent.monkey
    #gevent.monkey.patch_all()
    seg_size = 0
    ...
    for res in adata:
        #print("Res : " + str(res))
        for ores in res.response_list:
            send_time = ores.timestamp
            send_data = ores.payload
            send_delay = send_time - prev_time
            if send_time != 0:
                # send_delay = (time_org + send_time) - time.time()
                if send_delay != 0:
                    time.sleep(send_delay)
                    #print("Waiting for " + str(send_delay) + "s")
                    #gevent.sleep(seconds=(send_delay))
            edata = send_data.encode('utf-8')
            c.sendall(edata)
            #print("Sent data of size " + str(len(edata)))
            prev_time = send_time
    return seg_size
        ...
        try:
            ...
            if rstatus == SUCCESS:
                ...
                if apply_adpt:
                    ...
                    rstatus = sserver_update_client_data_tx(app, addr, corr)
                    ...
                    if seg_count in DIS_SEG:
                        ...
                        is_dis_seg = False
                        blist = None
                    dsize += stream_stored_app_data(ss, cid, app, TX_WINDOW, is_dis_seg, blist)
                    pth = th
                    th = 0
                    seg_count += 1
                else:
                    ...
                    c += 1
                    # print("Previous Time:", time.asctime(time.localtime(time.time())))
                    dsize += stream_stored_app_data(ss, cid, app, TX_WINDOW, False, None)
...
def sserver_update_client_data_tx(app, addr, corr):
    rstatus = ERROR
    client_port = addr[1]
    client_addr = addr[0]
    client_id = sserver_get_client_id(client_port, client_addr)
...
def start_client_thread(c, addr):
    ...
    c = sserver_get_ssl_socket(c) # Not required for StreamServer
...
def setup_sserver(port, addr):
    ...
    try:
        ...
        s = gevent.server.StreamServer((addr, port), handle, spawn=pool,
                                keyfile='../SSL_cert/1/server.key',
                                certfile='../SSL_cert/1/server.cert')
        s.init_socket()
        s.socket.setsockopt(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1)
        s.socket.setsockopt(socket.SOL_TCP, socket.TCP_NODELAY, 1)
        s.socket.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
        s.socket.settimeout(0.01)
        s.start()
        rstatus = SUCCESS
        '''
        # self.instance = (str(self.instance[0]), int(server.address[1]))
...
def handle (connection, address):
    start_client_thread(connection, address)


def load_Qs(app, pfile, serialize='pickle'):
    '''
    ...
    '''
    pickle_file = pfile
    output_data = "App : " + str(app) + "  Pickle file : " + str(pickle_file) + "\n"
    #print(output_data)

    if serialize == 'pickle':
        with open(pickle_file, 'rb') as f:
            Q, tmpLUT, tmpgetLUT, udpServers, tcpServerPorts, replayName = pickle.load(f)
    elif serialize == 'json':
...
def load_Qs_cl(app, pfile, serialize='pickle'):
    '''
    This loads and de-serializes all necessary objects.
    ...
    for p in Q:
        ...
        # fp = open('./temp.txt', 'a')
        # output_data = "Replay client : p in Q (hex decode): " + str(p.payload) + "\n"
...
def rserver_get_slist(dlist):
    slist = []
    for p in dlist:
        # print("P :" + str(p))
        # print("p response_len :" + str(p.response_len))
        if p.response_len != 0:
            slist.append(p.response_len)
    return slist


def rserver_add_length(dlist, dlist_cl):
    rlist = dlist.copy()
    slist = rserver_get_slist(dlist_cl)
    i = 0
    lline = slist[0]
    clline = 0
    for res in rlist:
        for ores in res.response_list:
            clline += len(ores.payload)
            if clline >= lline:
                fdata = "Content-Length: " + str(slist[i]) + ":"
                ores.payload = fdata + ores.payload
                i += 1
                clline = 0
                if len(slist) <= i:
                    break
...
def sserver_fill_app_data_p(app, pfile, pfile_cl):
    Qs_cl = load_Qs_cl(app, pfile_cl)
    Qs = load_Qs(app, pfile)
    Qs = rserver_add_length(Qs, Qs_cl)
    #print("Qs : " + str(Qs) + "\n")
    return Qs, Qs_cl
...
def sserver_print_app_data(data, cl_data):
    count = 1
    fp = open("app_data.txt", 'w')
    for res in data:
        for ores in res.response_list:
            output_data = "Count : " + str(count) + "  " + "Response : " + str(ores.timestamp) + " : "
            output_data += "Payload : " + str(ores.payload) + "\n"
            fp.write(output_data)
            count += 1
        output_data = "\nCompleted " + str(count) + "\n"
        fp.write(output_data)
    fp.close()
    count = 0
    fp = open('./client_data.txt', 'w')
    for p in cl_data:
        output_data = "Count : " + str(count) + " : " + str(p) + "\n"
        fp.write(output_data)
        count += 1
    fp.close()
...
def sserver_setup_app_data(app):
    pfile = app_to_file_map[app]
    pfile_cl = app_to_clfile_map[app]
    app_data, cl_data = sserver_fill_app_data_p(app, pfile, pfile_cl)
    sserver_print_app_data(app_data, cl_data)
    app_to_data[app] = app_data
    client_to_data[app] = cl_data
